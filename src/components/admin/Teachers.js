...
const Teachers = ({ createAlert }) => {
  ...
  const onClick = (e) => {
    ...
    if (firstName === '' || lastName === '' || username === '' || email === '')
      return createAlert('One or more field is missing.', 'danger', 5000);
    if (useDefaultPassword === false && password === '')
      return createAlert('Please enter a custom password', 'danger', 5000);
    axios
      .post(
        process.env.NODE_ENV === 'development'
      ...
      <table className='table' style={{ marginTop: '20px' }}>
        ...
        <tbody>
          {teachers.map((teacher) => (
            <tr key={teacher.teacher_id} id={teacher.teacher_id}>
              <td>{teacher.first_name.charAt(0).toUpperCase() + teacher.first_name.slice(1)}</td>
              <td>{teacher.last_name.charAt(0).toUpperCase() + teacher.last_name.slice(1)}</td>
              <td>{teacher.username}</td>
              <td>
                {teacher.password === '$2a$10$21tQ9rVJpGkax0vIN8gUs.Q0TtxGasogeeH5eRlKgnaq2nEwhX2PS'
                  ? 'Default'
                  : 'Custom'}
              </td>
              <td>{teacher.email}</td>
              ...
              <td>
                <Dropdown>
                  ...
                  <Dropdown.Menu id={teacher.teacher_id}>
                    ...
                    <Dropdown.Divider />
                    <Dropdown.Item
                      onClick={() => alert('This function is currently unavailable.')}
                      style={{ color: 'red' }}
                    >
                      Delete Teacher
                    </Dropdown.Item>
