...
const EditStudent = ({ createAlert }) => {
      ...
      .then(() => {
        axios
          .get(
            ...
            { headers: { token: token.current, student_id } }
          )
          .then((sdata) => {
            setFirstName(sdata.data[0].first_name);
            setLastName(sdata.data[0].last_name);
            setUsername(sdata.data[0].username);
          });
      })
      .catch((err) => {
  ...
  const onUpdate = (e) => {
    ...
    axios
      .post(
        process.env.NODE_ENV === 'development'
          ? '/api/teacher/student/edit'
          : 'https://spelling-tests-backend.herokuapp.com/api/teacher/student/edit',
        {
